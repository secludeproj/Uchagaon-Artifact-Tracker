  ...
  const downloadPdfLabels = async () => {
    ...
    try {
      ...
      for (let i = 0; i < itemsToPrint.length; i++) {
        ...
        doc.setDrawColor(180, 170, 150); // Muted gold-gray
        doc.setLineWidth(0.4);
        doc.setLineDashPattern([2, 2], 0);
        doc.roundedRect(x, y, colWidth, rowHeight, 3, 3, "S");
        doc.setLineDashPattern([], 0); // Reset dash

        // 2. Draw interior content
        // Watermark header
        doc.setFont("times", "bold");
        doc.setFontSize(6.5);
        doc.setTextColor(140, 130, 115);
        doc.text("SECLUDE PALACE HOTEL HERITAGE", x + 5, y + 7);
        ...
        doc.text(`ID : ${item.id}`, x + 5, metaY + 4.5);
        doc.text(`ORIG: ${item.originalLocation || "Palace"}`, x + 5, metaY + 9);

        // Lease Compliance Pill Tag
        doc.setDrawColor(210, 205, 190);
        doc.setFillColor(248, 247, 244);
        doc.rect(x + 5, metaY + 13, 34, 5, "FD");

        doc.setFont("courier", "bold");
        doc.setFontSize(6.5);
        doc.setTextColor(110, 100, 90);
        doc.text("LEASE COMPLIANCE TAG", x + 7, metaY + 16.5);
        ...
        // Draw inner frame for the QR code
      ...
      doc.save(`SECLUDE-PALACE-QR-LABELS.pdf`);
    } catch (err) {
