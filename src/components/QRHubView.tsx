...
export default function QRHubView({ artifacts, onBack }: QRHubViewProps) {
  ...
  const downloadPdfLabels = async () => {
    ...
    try {
      ...
      // Real Seclude logo — shared asset, see src/lib/secludeLogoAsset.ts
      const logoDataUrl = SECLUDE_LOGO_DATA_URL;
      ...
      const rowGap = 8;
      const startX = 16;
      const startY = groupByRoomEnabled ? 24 : 18;
      const perPage = 8;

      // When grouped, order by room (physical block/room order) instead of
      // insertion order, and start each room on a fresh page so the printed
      // stack can be carried and applied room-by-room.
      const orderedItems: { item: Artifact; room: string }[] = groupByRoomEnabled
        ? groupByRoom(itemsToPrint).flatMap((g) => g.items.map((item) => ({ item, room: g.room })))
        : itemsToPrint.map((item) => ({ item, room: item.currentLocation || "" }));
      ...
      for (let i = 0; i < orderedItems.length; i++) {
        ...
        // 2. Logo at top-left (small, fits cleanly above text) — real logo aspect ratio ~3.17:1
        const logoH = 6;
        if (logoDataUrl) {
          try {
  ...
  const selectedGroups = groupByRoomEnabled
    ...
    : [{ block: "", room: "", items: selectedArtifactsList }];
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center gap-2 min-w-0">
                              {selected ? <CheckSquare className="w-4 h-4 text-[#3b5249] shrink-0" /> : <Square className="w-4 h-4 text-gray-300 shrink-0" />}
            ...
            <div className="space-y-8">
              {selectedGroups.map((g) => (
                <div key={g.room || "flat"} className="space-y-3 break-inside-avoid">
                  {groupByRoomEnabled && (
                    <h3 className="font-mono text-xs font-bold uppercase tracking-widest text-[#3b5249] border-b border-[#dfd6be] pb-1">
                      {g.block} — {g.room}
                    </h3>
                  )}
